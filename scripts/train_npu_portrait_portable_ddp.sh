...
[[ -f "${COCO_PERSON_MANIFEST}" && -f "${WIDERFACE_MANIFEST}" && -f "${FACESCAPE_MANIFEST}" && -f "${MVHUMANNET_MANIFEST}" ]] || { echo "Missing portrait/outdoor manifests." >&2; exit 1; }
...
COCO_WEIGHT="${COCO_WEIGHT:-1.0}"; WIDERFACE_WEIGHT="${WIDERFACE_WEIGHT:-1.0}"
FACESCAPE_WEIGHT="${FACESCAPE_WEIGHT:-1.0}"; MVHUMANNET_WEIGHT="${MVHUMANNET_WEIGHT:-1.0}"
...
exec torchrun --standalone --nproc_per_node="${WORLD_SIZE}" --master_port="${MASTER_PORT}" -m unisharp.cli train-feature \
  ...
  --unik3d-backbone "${UNIK3D_BACKBONE}" --initializer-stride 2 --coco-person-manifest "${COCO_PERSON_MANIFEST}" --widerface-manifest "${WIDERFACE_MANIFEST}" --facescape-manifest "${FACESCAPE_MANIFEST}" --mvhumannet-manifest "${MVHUMANNET_MANIFEST}" \
  ...
  --dataset-weight-coco-person "${COCO_WEIGHT}" --dataset-weight-widerface "${WIDERFACE_WEIGHT}" --dataset-weight-facescape "${FACESCAPE_WEIGHT}" --dataset-weight-mvhumannet "${MVHUMANNET_WEIGHT}" --lambda-percep 0 --vis-every 0 "$@"
